...
function recursiveBinarySearch(arr, target, left, right) {
    ...
    console.log("target: " + target + " ,left: " + left + " , right: " + right + " , mid: " + mid)
    if (left > right){
        return -1
    }
    if (target === arr[mid]) {
...
function test(arr, num){
    ...
    array[1] = num;
    array[2] = 0;
    array[3] = arr.length-1;
    return array
}

let array = test([1,2,2,2,2,2,2,4,5,5,5,5,6,6,6], 7);
console.log(array);
console.log(recursiveBinarySearch(array[0],array[1],left = array[2],right = array[3]));
